...
import diaryapp.{AppConfig, FaunaConfig}
import diaryapp.persistence.model.{Entity, Page, PaginationOptions}
...
trait FaunaRepository[A <: Entity] extends Repository[A] with FaunaRepository.Implicits  {
  ...
  override def remove(id: String): Future[Option[A]] = {
    val result = client.query(
      Select(
        "data",
        Delete(Ref(Collection(className), id))
      )
    )
  ...
  override def find(id: String): Future[Option[A]] = {
    val result = client.query(
      Select("data", Get(Ref(Collection(className), id)))
    )
  ...
  def findAll()(implicit po: PaginationOptions): Future[Page[A]] = {
    val beforeCursor = po.before.map(id => Before(Ref(Collection(className), id)))
    val afterCursor = po.after.map(id => After(Ref(Collection(className), id)))
  ...
  /**
    ...
    * a transactional query for performing a valid [[Repository#save Repository#save]] operation.
    *
    * @param id the Id of the Entity to be saved
    * @param data the data of the Entity to be saved
    * @return a query for performing a valid [[Repository#save Repository#save]] operation
   * @see [[https://docs.fauna.com/fauna/current/reference/queryapi/write/create Create]]
    * @see [[https://docs.fauna.com/fauna/current/reference/queryapi/write/replace Replace]]
    * @see [[https://docs.fauna.com/fauna/current/reference/queryapi/basic/if If]]
    * @see [[https://docs.fauna.com/fauna/current/reference/queryapi/logical/exists Exists]]
  ...
  protected def saveQuery(id: Expr, data: Expr): Expr =
    Select(
      "data",
      If(
        Exists(Ref(Collection(className), id)),
        Replace(Ref(Collection(className), id), Obj("data" -> data)),
        Create(Ref(Collection(className), id), Obj("data" -> data))
      )
    )
...
/**
  ...
  */
final case class FaunaSettings(appConfig: FaunaConfig) {
  val endpoint = appConfig.endpoint
  val secret = appConfig.faunaKey
}

object FaunaSettings {
  val layer: URLayer[FaunaConfig, FaunaSettings] =  ZLayer.fromFunction(FaunaSettings.apply _)
}
...
case class FaunaDatabaseClient(faunaSettings: FaunaSettings) extends FaunaDbClient {
  override def get: FaunaClient = FaunaClient(faunaSettings.secret, faunaSettings.endpoint)
}

object FaunaDatabaseClient {
  val layer: URLayer[FaunaSettings,FaunaDbClient] =
    ZLayer.fromFunction(FaunaDatabaseClient.apply _)
}
